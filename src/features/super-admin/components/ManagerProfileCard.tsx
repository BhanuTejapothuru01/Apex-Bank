...
export default function ManagerProfileCard({
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-2xl bg-[#070c2e]/80 p-4 transition-all duration-300 flex flex-col justify-between box-border select-none w-full max-w-full h-[200px] text-slate-100 border shadow-2xl cursor-pointer ${
        isSelected
          ? 'border-[#d4af37] ring-2 ring-[#d4af37]/50 bg-[#121c4b]'
          : 'border-[#17235a]/60 hover:border-[#d4af37]'
      }`}
    >
      ...
      <div className="flex items-start justify-between w-full mt-1.5 font-sans">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-full bg-[#131b45] text-[#d4af37] border border-[#1b2559] flex items-center justify-center font-black text-sm select-none shadow-sm shrink-0 pb-0.5">
            {manager.avatarSeed}
          </div>
          <div className="min-w-0">
            <h4 className="text-sm font-black text-white leading-tight tracking-tight truncate w-full max-w-[150px]">
              {manager.name}
            </h4>
      ...
      <div className="space-y-1 mt-2.5 text-left flex-1 min-w-0 font-sans">
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-[#8496bf]">
          <MapPin size={12} className="text-[#8496bf]/80 shrink-0" />
      ...
      <div className="pt-2 border-t border-[#1b2559]/30 flex items-center justify-between text-[#8496bf] font-bold font-sans">
        <span className="text-[10px] uppercase tracking-wider text-[#8496bf] flex items-center gap-1">
          <Users size={11} className="text-[#d4af37]" />
          <span>Staff Managed</span>
        </span>
        <span className="font-mono text-xs font-black text-slate-300 bg-amber-500/5 px-2 py-0.5 rounded border border-amber-500/10">
