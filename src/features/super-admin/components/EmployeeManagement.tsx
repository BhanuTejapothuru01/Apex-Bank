  ...
  return (
    <div id="employee-management-view" className="space-y-6 select-none font-sans">
      ...
      <div 
        ...
        className="w-full bg-[#FCE7F3]/90 border border-[#F9A8D4] rounded-2xl p-4 sm:p-5 flex flex-col gap-5 shadow-2xl relative z-30 font-sans text-[#4A044E]"
      >
        {/* Row 1: Search and Adding Managers */}
        <div className="flex flex-col md:flex-row items-center gap-4 w-full">
          {/* A. Search Branch Manager */}
          <div className="relative w-full md:flex-[2.5]">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#BE185D]/75 w-4 h-4" />
            <input
              id="manager-search"
              type="text"
              ...
              className="w-full bg-[#FFF1F5] border border-[#F9A8D4] focus:border-[#d4af37]/60 text-[#4A044E] placeholder-[#EC4899]/45 text-xs pl-10 pr-4 py-3 rounded-xl outline-none transition-all placeholder:font-medium"
            />
          </div>
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-5 border-t border-[#F9A8D4]/30 pt-4 w-full">
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer flex items-center gap-2 shrink-0 border active:scale-95 ${
                    isActive
                      ? 'bg-gradient-to-r from-violet-600 to-indigo-600 text-[#4A044E] border-violet-500/40 shadow-lg shadow-violet-600/20 font-black'
                      : 'bg-[#FFF1F5] text-[#BE185D]/75 border-[#F9A8D4] hover:border-[#8496bf]/45 hover:text-[#4A044E]'
                  }`}
                >
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-md font-mono font-bold ${isActive ? 'bg-white/20 text-[#4A044E]' : 'bg-white/5 text-[#9D174D]/85'}`}>
                    {count}
                  </span>
                </button>
              );
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div className="relative w-full sm:w-[150px]">
              <select
                ...
                className="w-full bg-[#FFF1F5] border border-[#F9A8D4] focus:border-[#d4af37]/60 text-[#4A044E] font-bold text-xs pl-3.5 pr-8 py-2.5 rounded-xl outline-none cursor-pointer appearance-none h-[38px]"
              >
                <option value="All" className="bg-[#FCE7F3] text-[#4A044E]">{t("Filter Region")}</option>
                {uniqueRegions.filter(r => r !== 'All').map(reg => (
                  <option key={reg} value={reg} className="bg-[#FCE7F3] text-[#BE185D]/75">{reg}</option>
                ))}
              </select>
            ...
            <div className="relative w-full sm:w-[155px]">
              <select
                ...
                className="w-full bg-[#FFF1F5] border border-[#F9A8D4] focus:border-[#d4af37]/60 text-[#4A044E] font-bold text-xs pl-3.5 pr-8 py-2.5 rounded-xl outline-none cursor-pointer appearance-none h-[38px]"
              >
                <option value="Name" className="bg-[#FCE7F3] text-[#4A044E]">{t("Sort by Name")}</option>
                <option value="Rating" className="bg-[#FCE7F3] text-[#4A044E]">{t("Sort by Rating")}</option>
                <option value="ID" className="bg-[#FCE7F3] text-[#4A044E]">{t("Sort by ID")}</option>
              </select>
              <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#d4af37] pointer-events-none">
                <ChevronDown size={14} className="text-[#d4af37]" />
              </span>
            ...
            <div className="bg-[#FFF1F5] border border-[#F9A8D4] rounded-xl p-1 flex items-center gap-1 shrink-0 h-[38px] w-full sm:w-[160px] justify-center">
              <button
                type="button"
      ...
      <div className="flex-1">
        {filteredAndSortedManagers.length > 0 ? (
          viewMode === 'list' ? (
            <div className="p-3 sm:p-5 lg:p-6 rounded-2xl border border-[#F9A8D4] bg-[#FCE7F3]/90 shadow-2xl relative text-[#4A044E]">
              <div className="mb-4">
                <h3 className="text-base font-bold text-[#4A044E]">Consolidated Branch Command Ledger</h3>
                <p className="text-[#9D174D]/80 text-xs">A comprehensive roster of corporate executive managers assigned to brand nodes.</p>
              </div>
                    ...
                    {filteredAndSortedManagers.map((mgr) => {
                      ...
                      return (
                        <tr 
                          ...
                          className={`text-xs hover:bg-[#FBCFE8]/70 transition-colors cursor-pointer group ${
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-2.5">
                              <div className="w-8 h-8 rounded-full shrink-0 bg-[#FBCFE8] text-[#d4af37] border border-[#F9A8D4] flex items-center justify-center font-black text-xs select-none shadow-sm pb-0.5">
                                {mgr.avatarSeed}
                              </div>
                              <div>
                                <span className="font-extrabold text-[#4A044E] group-hover:text-[#d4af37] transition-colors block">{mgr.name}</span>
                                <span className="text-[9px] text-[#BE185D]/75 font-medium leading-none md:hidden">{mgr.designation}</span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="py-3 px-4 font-bold text-[#831843] whitespace-nowrap max-w-[150px] truncate hidden md:table-cell" title={mgr.designation}>
                            {mgr.designation}
                          </td>
                          <td className="py-3 px-4 font-medium text-[#BE185D]/75 whitespace-nowrap max-w-[150px] truncate hidden lg:table-cell" title={mgr.department}>
                            {mgr.department}
                          </td>
                          <td className="py-3 px-4">
                            <div className="flex flex-col">
                              <span className="font-bold text-[#4A044E] whitespace-nowrap text-xs">{mgr.branchName}</span>
                              <span className="text-[10px] text-[#9D174D]/80 whitespace-nowrap">{mgr.branchLocation}</span>
                            </div>
                          </td>
                          <td className="py-3 px-4 whitespace-nowrap">
                            <div className="flex items-center gap-1 font-bold font-mono">
                              <span className="text-amber-400 font-extrabold">★</span>
                              <span className="text-[#4A044E]">{mgr.rating}</span>
                            </div>
                          </td>
                          <td className="py-3 px-4 hidden sm:table-cell">
                            <div className="flex items-center justify-center gap-2">
                              <div className="w-16 bg-[#F9A8D4]/40 rounded-full h-1.5 overflow-hidden border border-[#F9A8D4]/40">
                                <div 
                                  className="bg-gradient-to-r from-amber-400 to-[#d4af37] h-1.5 rounded-full" 
                                  style={{ width: `${capacityScore}%` }}
                                />
                              </div>
                              <span className="font-mono text-[10px] text-[#4A044E] font-bold">{capacityScore}%</span>
                            </div>
                          </td>
          ...
          <div className="text-center py-24 bg-[#FCE7F3]/90 border border-[#F9A8D4] rounded-3xl animate-pulse">
            <Search className="mx-auto text-[#BE185D]/75 mb-4" size={54} />
            <h4 className="text-base font-black uppercase text-[#4A044E] tracking-widest">{t("No Manager Profile Found")}</h4>
            <p className="text-xs text-[#9D174D]/80 mt-1.5">Check spelling query or clear status filters.</p>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {showAddModal && (
          <div 
            ...
            className="fixed inset-0 bg-black/35 backdrop-blur-[6px] flex items-center justify-center z-[99999] p-4 text-[#4A044E] font-sans"
          >
            <motion.div 
              ...
              className="w-[75%] h-[80vh] bg-[#FCE7F3] border border-[#F9A8D4]/80 rounded-[28px] shadow-2xl overflow-hidden relative flex flex-col"
            >
              ...
              <div className="p-5 border-b border-[#F9A8D4]/50 flex items-center justify-between bg-[#FFF1F5]/50">
                ...
                <button 
                  ...
                  className="p-1.5 rounded-lg text-[#9D174D]/85 hover:text-[#4A044E] hover:bg-white/5 transition-all cursor-pointer"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateManager} className="p-6 space-y-4 text-xs select-none flex-1 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Full Name")}:</label>
                    <input
                      type="text"
                      required
                      value={newManagerForm.name}
                      onChange={(e) => setNewManagerForm({ ...newManagerForm, name: e.target.value })}
                      className="w-full bg-[#FFF1F5] border border-[#F9A8D4]/50 text-[#4A044E] p-2.5 rounded-lg outline-none focus:border-[#d4af37]/60"
                      placeholder="E.g. Mohammed Rahman"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Corporate Email")}:</label>
                    <input
                      type="email"
                      required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Mobile Number")}:</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Gender")}:</label>
                    <select
                      value={newManagerForm.gender}
                      onChange={(e) => setNewManagerForm({ ...newManagerForm, gender: e.target.value })}
                      className="w-full bg-[#FFF1F5] border border-[#F9A8D4]/50 text-[#4A044E] p-2.5 rounded-lg outline-none cursor-pointer focus:border-[#d4af37]/60"
                    >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Assigned Branch Node")}:</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Branch Code")}:</label>
                    <input
                      type="text"
                      disabled
                      value={newManagerForm.branchCode}
                      className="w-full bg-[#FFF1F5] border border-white/15 text-[#9D174D]/85 p-2.5 rounded-lg outline-none font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Branch Location")}:</label>
                    <input
                      type="text"
                      disabled
                      value={newManagerForm.branchLocation}
                      className="w-full bg-[#FFF1F5] border border-white/15 text-[#9D174D]/85 p-2.5 rounded-lg outline-none"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Department")}:</label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Role / Designation")}:</label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1">
                    <label className="text-[#9D174D]/85 font-bold uppercase tracking-wide">{t("Employee Grade")}:</label>
                    <select
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-[#F9A8D4]/50">
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="px-4 py-2 hover:bg-white/5 text-[#9D174D]/85 text-xs font-semibold rounded-lg transition-all"
                  >
                    {t("Cancel")}
                  </button>
                  <button
