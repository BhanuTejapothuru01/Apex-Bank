      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 select-none">
        ...
        <div className="p-6 rounded-2xl border border-[#17235a]/60 bg-[#070c2e]/80 shadow-2xl space-y-4">
          <div>
            ...
            <h3 className="text-base font-bold text-white mt-1">{t('sovereign_mfa', 'Sovereign Compliance & MFA settings')}</h3>
            <p className="text-xs text-[#8496bf]">{t('mfa_desc', 'Authorize hardware security modules and multi-factor session handshakes.')}</p>
          </div>

          <div className="space-y-4 pt-2 text-xs">
            
            <div className="flex items-center justify-between p-3.5 rounded-xl border border-[#141b4a] bg-[#04081c]/60">
              <div>
                <h4 className="font-bold text-white">{t('enforce_mfa', 'Enforce Hardware MFA Key tokens')}</h4>
                <p className="text-[10px] text-[#556994] mt-0.5">{t('yubikey_req', 'Require Yubikey hardware handshakes on transactions.')}</p>
              </div>
              <button 
                id="toggle-mfa-setting"
                onClick={() => setMfaStatus(!mfaStatus)}
                className={`w-12 h-6 rounded-full p-1 transition-colors cursor-pointer ${mfaStatus ? 'bg-emerald-500' : 'bg-[#18214d]'}`}
              >
            ...
            <div className="space-y-1">
              <label className="text-slate-400 font-bold uppercase tracking-wide">{t('session_timeout', 'Session Idle Timeout (Minutes):')}</label>
              <input 
                id="session-timeout-input"
                type="number"
                value={sessionTimeout}
                onChange={(e) => setSessionTimeout(Number(e.target.value))}
                className="w-full bg-[#0a1135] border border-[#1b2557] focus:border-[#d4af37]/60 text-white p-2.5 rounded-lg text-xs outline-none font-mono"
              />
            </div>

            <div className="space-y-1">
              <label className="text-slate-400 font-bold uppercase tracking-wide">{t('clearance_tier', 'Current Profile Clearance Tier:')}</label>
              <select
                id="clearance-level-select"
                value={clearanceLevel}
                onChange={(e) => setClearanceLevel(Number(e.target.value))}
                className="w-full bg-[#0a1135] border border-[#1b2557] focus:border-[#d4af37]/60 text-white p-2.5 rounded-lg text-xs outline-none cursor-pointer"
              >
            ...
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer"
            >
        ...
        <div className="p-6 rounded-2xl border border-[#17235a]/60 bg-[#070c2e]/80 shadow-2xl relative flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <h3 className="text-sm font-bold text-white">{t('crypto_key_hashes', 'Apex cryptographic key hashes')}</h3>
